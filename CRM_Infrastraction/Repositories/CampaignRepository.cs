    ...
    public class CampaignRepository : ICampaignRepository
    {
        ...
        public async Task<List<CampaignDto>> SearchCampaignAsync(string searchKeyWord)
        {
            ...
            if(campaigns.Any())
            {
                ...
                {
                    CampaignDto campaignDto = new CampaignDto() { Id = item.Id,Budget = item.Budget,Description = item.Description,
                        StartDate = item.StartDate,EndDate = item.EndDate,Name = item.Name,UserId = item.UserId};
